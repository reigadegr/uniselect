...
compile_start(){
    ...
    -fno-rtti -fvisibility=hidden -static-libgcc -static-libstdc++  \
     -static-openmp -static-libsan -liconv \
    ...
    $(pwd)/*.cpp -o $(dirname "$0")/GB2312ToUTF8 && echo "*编译完成*" || exit 1
    /data/data/com.termux/files/usr/bin/aarch64-linux-android-strip $(dirname "$0")/GB2312ToUTF8
    chmod +x $(dirname "$0")/GB2312ToUTF8
